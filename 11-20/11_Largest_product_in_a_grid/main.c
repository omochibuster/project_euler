...
int main(void){
	...
	int value[20][20] = {{8,2,22,97,38,15,0,40,0,75,4,5,7,78,52,12,50,77,91,8},
	...
	for(i = 0; i < 20; i ++){
		for(j = 0; j < 20; j ++){
			if(i < 16){//縦
				tmp = value[i][j] * value[i + 1][j] *
				value[i + 2][j] * value[i + 3][j];;
				if(max < tmp)
					max = tmp;
				if(j < 16){//右
					tmp = value[i][j] * value[i + 1][j + 1] *
						value[i + 2][j + 2] * value[i + 3][j + 3];
				if(max < tmp)
						max  = tmp;
				}
				if(j > 3){//左
			...
			if(j < 16){//ななめ
				tmp = value[i][j] * value[i][j + 1] * 
				value[i][j + 2] * value[i][j + 3];
				if(max < tmp)
					max = tmp;
			}
		}
